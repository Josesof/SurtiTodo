...
public class OcrdController {
	...
	List<OCRD> findByOcrdId(String CardCode){
		...
		public String crear(
				...
				model.addAttribute("ocrds", ocrd);
			
				
			return "datosmaestros";
		}
 @PostMapping(value = "/datosmaestro")
	public String insertardata(@RequestParam(value = "CardCode", required = false) String CardCode,
		...
		try {
			...
			crd1.setCardCode(CardCode);
			crd1.setAddress(Address);
			crd1.setCity(City);
			crd1.setCountry(Country);
			crd1.setU_HBT_DirMM(U_HBT_DirMM);
			ocrdImpl.saveSocio(CardCode, CardName, GroupCode, LicTradNum, E_Mail, Phone1, AddID, SlpCode, U_HBT_RegTrib,
					...
					U_GSP_GENDER, U_CatalogosActivos, U_CRMContrasena, Address, City, Country, U_HBT_DirMM);
	...
	/*
	 * @GetMapping(value = "/buscar/{termino}")
	public Collection<OCRD> buscar(@RequestParam(name = "termino", required = false) String termino, Model model,
			RedirectAttributes flash) {
		return ocrdImpl.buscaPorCardCode(termino);
